...
$( document ).ready(function() {
    ...
    $.ajax({
        url : "{{ url('api/order'.'?page='.app('request')->input('page')) }}",
        type: "get",
        headers: {
        ...
        success: function (result) {
            ...
            tr.append(th.html('Title').clone());
            tr.append(th.html('Publisher').clone());
            tr.append(th.html('Price').clone());
            thead.html(tr.clone());
            ...
            for (var i = 0; i < result.data.length; i++) {
                ...
                tr.append(th.html(result.data[i].game.title).clone());
                tr.append(th.html(result.data[i].game.publisher).clone());
                tr.append(th.html(result.data[i].game.price + "$").clone());
                tbody.html(tr.clone());
            ...
            for (let i = 1; i <= result.last_page; i++) {
                page.find('a').attr('href', "{{ url('orderhistory') }}?page="+i).html(i);
